...
在 **Skill / General 模式**开始构建上下文时，blockcell 会生成一个**记忆摘要**并注入系统提示词；**Chat 模式当前不注入 Memory Brief**：
...
因此 Skill / General 任务可以直接利用相关背景信息；Chat 模式目前仍按无 Memory Brief 的轻量上下文运行。
...
- **全文搜索**：FTS5 支持中文关键词搜索
- **混合检索**：可选向量索引补充语义召回
- **智能注入**：Skill / General 模式注入最相关的记忆摘要；Chat 模式当前不注入
- **自动维护**：过期清理、软删除回收站
- **隐私安全**：完全本地，不上传
